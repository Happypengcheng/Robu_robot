...
//	#define ARROWBLU 0X00FF0000
	...
	int FlagPause;							// lipan_0 代表暂停状态 1代表正常状态
	...
	MotionParemeter Temp_Line[mTotal];	// 保存接收服务器过来的指令Line
	...
	void SendIGVRealTimeInfo();		// 定时向服务端发送小车实时信息

	// 发送轨迹校准相关的数据到后台
	void SendIGVTrackFixInfo(CString markID1, int x1, int y1, double deltah1, 
		double deltax1, double deltay1, double speed1, double x2, double y2,
		double k2, CString markID3, int x3, int y3, double deltah3, double deltax3, double deltay3,
		int heading, int motionDirection);
	int m_iTimeCounter;				// 计数器，OnTimer每执行10次回传一次自身位置和状态信息
	int CurLineNo;					// 记录当前的Line序号
	bool IsEmgcBrake;					// 是否紧急制动
...
protected:
	...
	afx_msg void OnButton_PauseAndRecovery();
	afx_msg void OnButton_SlowDown();
	...
	afx_msg void OnButton_EmergencyBraking();
